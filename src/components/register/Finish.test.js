...
describe("Finish Component", () => {
  ...
  test("shows error message for less than 10 characters on submit", () => {
    ...
    expect(
      screen.getByText("Please describe yourself in at least 10 characters.")
    ).toBeInTheDocument();
  });
  ...
  test("dispatches saveTextArea and shows modal on valid submit", async () => {
    renderComponent();
    fireEvent.change(screen.getByRole("textbox"), {
      target: { value: "Valid text more than 10 characters" },
    });
    fireEvent.click(screen.getByText("Save"));
    await waitFor(() => {
      expect(store.dispatch).toHaveBeenCalledWith(
        saveTextArea("Valid text more than 10 characters")
      );
  ...
  test("toggles modal visibility on Cancel click", () => {
    ...
    expect(
      screen.queryByText("Registered Successfully")
  ...
  test("does not dispatch saveTextArea or prevStep on Skip button click", () => {
    ...
    expect(store.dispatch).not.toHaveBeenCalledWith(
      saveTextArea(expect.any(String))
    );
